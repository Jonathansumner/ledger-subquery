...
// }
//
export async function handleEventEmpty(event: CosmosEvent): Promise<void> {
...
export async function handleEvent(event: CosmosEvent): Promise<void> {
  logger.info(`${JSON.stringify(event?.tx, null, 2)}`);
  logger.info(`${JSON.stringify(event?.event, null, 2)}`);
...
export async function handleEventSuggested(event: CosmosEvent): Promise<void> {
  const sanitize = (value: unknown) => {
    const json = JSON.stringify(value);
    return json.substring(1, json.length - 1);
  };
